      ...
      {filtered.length === 0 ? (
        ...
        filtered.map((x) => (
          <div key={`${x.order}-${x.title}`} style={{ marginBottom: "0.75rem" }}>
            ...
            {(x.requirements?.badges?.length || x.requirements?.hms?.length) ? (
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 6 }}>
                {(x.requirements.badges || []).map((b) => (
                  <span key={b} style={{ fontSize: 12, padding: "2px 6px", borderRadius: 999, background: "rgba(79,172,254,0.15)" }}>
                    Badge: {b}
                  </span>
                ))}
                {(x.requirements.hms || []).map((h) => (
                  <span key={h} style={{ fontSize: 12, padding: "2px 6px", borderRadius: 999, background: "rgba(255,183,77,0.15)" }}>
                    HM: {h}
                  </span>
                ))}
              </div>
            ...
            {x.notes ? <div style={{ fontSize: "0.8rem", opacity: 0.75, marginTop: 4 }}>{x.notes}</div> : null}
          </div>
        ))
      )}
    </div>
